...
set "DIST=%REPO%\dist\ironbullet-v0.6.2-windows-x64"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "Get-ChildItem -LiteralPath '%DIST%' -File | Get-FileHash -Algorithm SHA256 | ForEach-Object { '{0}  {1}' -f $_.Hash.ToLower(), $_.Name } | Set-Content -Encoding ascii '%DIST%\release-manifest.sha256'; Compress-Archive -Path '%DIST%\*' -DestinationPath '%REPO%\dist\ironbullet-v0.6.2-windows-x64.zip' -Force"
if errorlevel 1 exit /b %errorlevel%
echo Built bundle: %REPO%\dist\ironbullet-v0.6.2-windows-x64.zip
